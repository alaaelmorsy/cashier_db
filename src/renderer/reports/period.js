  ...
  if(btnExcel){
    btnExcel.addEventListener('click', async () => {
      ...
      try{
        ...
        const filename = `period-report-${(period||'').replace(/[: ]/g,'_')||Date.now()}.xlsx`;
...
async function loadRange(startStr, endStr){
  ...
  try{
    ...
    try{
      ...
      if(typeof ReportProfitUtils !== 'undefined' && ReportProfitUtils.buildPaidBySale){
        paidBySale = ReportProfitUtils.buildPaidBySale(txs);
      } else {
        txs.forEach(tx => {
    ...
    try{
      const profit = (typeof ReportProfitUtils !== 'undefined' && ReportProfitUtils.calcProfitabilityTotals)
        ? ReportProfitUtils.calcProfitabilityTotals({
            ...
            costIncludesVat: settings.cost_includes_vat
          })
      ...
      console.error('Profitability calc failed (period):', err);
      set('costTotalPre', 0); set('salesTotalPre', 0); set('profitNetPre', 0);
      set('costTotalExVat', 0); set('salesTotalExVat', 0); set('profitNetExVat', 0);
    }
    ...
    try{
      ...
      if(soldItemsTbody){ soldItemsTbody.innerHTML = rows || `<tr><td colspan="6" class="muted">${noDataText}</td></tr>`; }
    }catch(_){ 
      const noDataText = __currentLang === 'ar' ? 'لا توجد بيانات' : 'No data';
      if(soldItemsTbody){ soldItemsTbody.innerHTML = `<tr><td colspan="6" class="muted">${noDataText}</td></tr>`; } 
    }
